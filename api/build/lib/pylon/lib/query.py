...
from pylon.database import Model, ModelNotFound, ModelValidationError
...
class WhereQuery(Generic[T], BaseSubQuery[T]):
    pass
...
class Query(Generic[T]):
    ...
    def insert(self) -> ExecuteQuery[T]:
        ...
        return ExecuteQuery[T](self.model, sql, args, len(self.model.__fields__))

    def select(self, alias=None) -> SelectQuery[T]:
        fields = ", ".join(
